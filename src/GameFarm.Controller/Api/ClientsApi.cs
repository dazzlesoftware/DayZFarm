    ...
    public static void MapClientsApi(this WebApplication app)
    {
        ...
        api.MapPost("", async (CreateClientRequest req, ClientOrchestrator orchestrator, CancellationToken ct) =>
        {
            try
            {
                ...
                var id = await orchestrator.CreateClientAsync(instance, req.ServerPassword, ct);
        ...
        api.MapPost("/{id:int}/game/start", (int id, ClientOrchestrator o, CancellationToken ct) => o.JoinAsync(id, ct));
        api.MapPost("/{id:int}/game/stop", (int id, ClientOrchestrator o, CancellationToken ct) => o.SendAgentCommandAsync(id, AgentApiRoutes.GameStop, ct));
        api.MapPost("/{id:int}/game/restart", (int id, ClientOrchestrator o, CancellationToken ct) => o.SendAgentCommandAsync(id, AgentApiRoutes.GameRestart, ct));
        ...
        // Guest-side plugins (see docs/PLUGINS.md) -- only a plugin's NAME ever travels over
        // HTTP; what it actually runs is fixed by an admin-authored JSON file already on disk
        // inside that VM, never anything supplied by this request.
        api.MapGet("/{id:int}/plugins", async (int id, ClientOrchestrator o, CancellationToken ct) =>
        {
            var plugins = await o.GetAgentPluginsAsync(id, ct);
            return plugins is null ? Results.NotFound() : Results.Ok(plugins);
        });

        api.MapPost("/{id:int}/plugins/{name}/run", (int id, string name, ClientOrchestrator o, CancellationToken ct) =>
            o.RunAgentPluginAsync(id, name, ct));

        // Agent binary self-update (see docs/AGENT-UPDATES.md) -- pushes the Controller's
        // currently uploaded package (POST /api/agent-package) to this client's agent.
        api.MapPost("/{id:int}/agent/update-package", (int id, ClientOrchestrator o, CancellationToken ct) =>
            o.UpdateAgentPackageAsync(id, ct));
        ...
        api.MapGet("/{id:int}/logs", async (int id, ClientOrchestrator o, CancellationToken ct) =>
        {
            ...
            var unavailableReason = agentLogs is not null ? null
                : view.VmStatus != GameFarm.Core.Models.VirtualMachineStatus.Running
                    ? "VM is not running."
        ...
        bulk.MapPost("/restart-game-selected", (int[] ids, ClientOrchestrator o, CancellationToken ct) =>
            BulkAgentAsync(ids, o, ct, id => o.SendAgentCommandAsync(id, AgentApiRoutes.GameRestart, ct)));
        ...
        bulk.MapPost("/update-agent-selected", (int[] ids, ClientOrchestrator o, CancellationToken ct) =>
            BulkAgentAsync(ids, o, ct, id => o.UpdateAgentPackageAsync(id, ct)));
        ...
        bulk.MapPost("/restart-game-all", async (ClientOrchestrator o, CancellationToken ct) =>
        {
            var all = await o.GetAllViewsAsync(ct);
            return await BulkAgentAsync(all.Select(v => v.Id).ToArray(), o, ct, id => o.SendAgentCommandAsync(id, AgentApiRoutes.GameRestart, ct));
        });
        ...
        bulk.MapPost("/update-agent-all", async (ClientOrchestrator o, CancellationToken ct) =>
        {
            var all = await o.GetAllViewsAsync(ct);
            return await BulkAgentAsync(all.Select(v => v.Id).ToArray(), o, ct, id => o.UpdateAgentPackageAsync(id, ct));
        });
    }
