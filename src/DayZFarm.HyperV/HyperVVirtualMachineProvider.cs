...
public sealed class HyperVVirtualMachineProvider : IVirtualMachineProvider
{
    ...
    public async Task CreateAsync(VirtualMachineCreateRequest request, CancellationToken ct = default)
    {
        ...
        if (!File.Exists(request.ParentVhdxPath))
            throw new FileNotFoundException($"Master VHDX not found at '{request.ParentVhdxPath}'.", request.ParentVhdxPath);
        ...
        // Library layout as scripts/Create-Client.ps1 instead of each installing DayZ from
