...
public sealed class WindowsWorkshopManager : IWorkshopManager
{
    ...
    private readonly IGameProfile _game;
    private readonly ILogger<WindowsWorkshopManager> _logger;

    public WindowsWorkshopManager(SteamManager steam, IGameProfile game, ILogger<WindowsWorkshopManager> logger)
    {
        _steam = steam;
        _game = game;
        _logger = logger;
    ...
    private IReadOnlyList<string> WorkshopContentDirectories() =>
        _steam.LibraryRoots()
            .Select(root => Path.Combine(root, "steamapps", "workshop", "content", _game.SteamAppId.ToString()))
