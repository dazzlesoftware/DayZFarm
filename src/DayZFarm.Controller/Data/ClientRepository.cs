...
/// <summary>
/// Lightweight SQLite-backed store for <see cref="DayZClientInstance"/> records. Uses
...
public sealed class ClientRepository : IClientRepository
{
    ...
    public ClientRepository(IOptions<DayZFarmOptions> options)
    {
    ...
    public async Task<int> AddAsync(DayZClientInstance instance, CancellationToken ct = default)
    {
        using var conn = new SqliteConnection(_connectionString);
    ...
    public async Task<IReadOnlyList<DayZClientInstance>> GetAllAsync(CancellationToken ct = default)
    {
        using var conn = new SqliteConnection(_connectionString);
        await conn.OpenAsync(ct);
        using var cmd = conn.CreateCommand();
        ...
        var list = new List<DayZClientInstance>();
        while (await reader.ReadAsync(ct))
            list.Add(Map(reader));
        return list;
    }

    public async Task<DayZClientInstance?> GetByIdAsync(int id, CancellationToken ct = default)
    {
        using var conn = new SqliteConnection(_connectionString);
        await conn.OpenAsync(ct);
        using var cmd = conn.CreateCommand();
    ...
    public async Task<DayZClientInstance?> GetByNameAsync(string name, CancellationToken ct = default)
    {
        using var conn = new SqliteConnection(_connectionString);
        await conn.OpenAsync(ct);
        using var cmd = conn.CreateCommand();
    ...
    public async Task UpdateAsync(DayZClientInstance instance, CancellationToken ct = default)
    {
        using var conn = new SqliteConnection(_connectionString);
        await conn.OpenAsync(ct);
        using var cmd = conn.CreateCommand();
    ...
    private static DayZClientInstance Map(SqliteDataReader reader) => new()
    {
