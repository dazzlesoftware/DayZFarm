...
.DESCRIPTION
    ...
    .NET runtime, then creates the D:\GameFarm directory layout and virtual switch. Never
    reboots automatically -- if enabling Hyper-V requires a reboot, this reports it and exits.

.PARAMETER RootDirectory
    Root storage directory for the farm (default D:\GameFarm).

.PARAMETER VirtualSwitchName
    Name of the Hyper-V virtual switch to create if missing (default GameFarmSwitch).

.PARAMETER SwitchType
    Hyper-V switch type: External (default, for internet/LAN access), Internal, or Private.

.EXAMPLE
    .\Install-Host.ps1 -RootDirectory D:\GameFarm -VirtualSwitchName GameFarmSwitch
#>
[CmdletBinding()]
param(
