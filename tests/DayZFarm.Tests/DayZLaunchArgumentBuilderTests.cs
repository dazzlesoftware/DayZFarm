...
public class DayZLaunchArgumentBuilderTests
{
    ...
    {
        var options = new DayZLaunchOptions { ServerAddress = "192.168.1.50", ServerPort = 2302 };
    ...
    {
        var withPassword = DayZLaunchArgumentBuilder.Build(new DayZLaunchOptions { ServerAddress = "a", ServerPort = 1, ServerPassword = "secret" });
        Assert.Contains("-password=secret", withPassword);

        var withoutPassword = DayZLaunchArgumentBuilder.Build(new DayZLaunchOptions { ServerAddress = "a", ServerPort = 1 });
    ...
    {
        var options = new DayZLaunchOptions { ServerAddress = "a", ServerPort = 1, AdditionalArguments = new List<string> { "-world=empty" } };
        var args = DayZLaunchArgumentBuilder.Build(options);
    ...
    {
        Assert.Throws<ArgumentException>(() => DayZLaunchArgumentBuilder.Build(new DayZLaunchOptions { ServerAddress = address, ServerPort = port }));
    }

    [Fact]
    public void BuildArgumentString_QuotesArgumentsContainingSpaces()
    {
        var options = new DayZLaunchOptions { ServerAddress = "a", ServerPort = 1, AdditionalArguments = new List<string> { "-profiles=C:\\My Profiles" } };
